<template><div><h1 id="kratos微服务轻松对接efk日志系统" tabindex="-1"><a class="header-anchor" href="#kratos微服务轻松对接efk日志系统"><span>Kratos微服务轻松对接EFK日志系统</span></a></h1>
...
<p>日志收集是微服务可观测性中不可或缺的一部分。日志对于调试问题和监视集群状况非常有用。</p>
<p>然而，集中化收集日志文件之后，并非就万事大吉了，我们还有一系列的问题需要解决：如何对日志文件进行分析和查询。哪些服务有报警和异常，这些也都需要有详细的统计。所以，以前出现线上故障时，经常会看到开发和运维人员下载服务的日志，并基于 Linux 下的一些命令（如 grep、awk 和 wc 等）进行检索和统计。这样的方式不仅工作量大，并且效率低下，而且对于要求更高的查询、排序和统计等操作，以及庞大的集群机器数量，无法胜任。</p>
<p>EFK(<a href="https://www.elastic.co/" target="_blank" rel="noopener noreferrer">Elasticsearch</a>、<a href="https://www.fluentd.org/" target="_blank" rel="noopener noreferrer">Fluentd</a>、<a href="https://www.elastic.co/kibana" target="_blank" rel="noopener noreferrer">Kibana</a>)是当下微服务中比较流行的分布式日志服务解决方案。以下，我们将就EFK应用于Kratos微服务，逐步展开讲解。</p>
<h2 id="efk-分布式日志系统" tabindex="-1"><a class="header-anchor" href="#efk-分布式日志系统"><span>EFK 分布式日志系统</span></a></h2>
<p>EFK 是一个完整的分布式日志收集系统，很好地解决了上述提到的日志收集难，检索和分析难的问题。</p>
<p>EFK 分别是指：</p>
<ul>
<li><a href="https://www.elastic.co/" target="_blank" rel="noopener noreferrer">Elasticsearch</a> 是一个分布式搜索引擎。具有高可伸缩、高可靠、易管理等特点。可以用于全文检索、结构化检索和分析，并能将这三者结合起来。Elasticsearch 基于 Lucene 开发，现在使用最广的开源搜索引擎之一，Wikipedia 、StackOverflow、Github 等都基于它来构建自己的搜索引擎。</li>
<li><a href="https://www.fluentd.org/" target="_blank" rel="noopener noreferrer">Fluentd</a> 是一个开源的数据收集器，我们可以在微服务集群节点上安装 Fluentd，通过获取容器日志文件、过滤和转换日志数据，然后将数据传递到 Elasticsearch 集群，在该集群中对其进行索引和存储。Fluentd的日志必须以JSON为载体，日志内容没有限制。</li>
<li><a href="https://www.elastic.co/kibana" target="_blank" rel="noopener noreferrer">Kibana</a> 是一个可视化化平台。Kibana 用于搜索、分析和可视化存储在 Elasticsearch 指标中的日志数据，是一个 Web 网页。Kibana 利用 Elasticsearch 的 REST 接口来检索数据，调用 Elasticsearch 存储的数据，将其可视化。它不仅允许用户自定义视图，还支持以特殊的方式查询和过滤数据。</li>
</ul>
<p>如果，我们将这一系列组件视为 MVC 模型，那么就是：</p>
<ul>
<li>Fluentd 对应逻辑控制 Controller 层</li>
<li>Elasticsearch 是一个数据模型 Model 层，</li>
<li>Kibana 则是视图 View 层。</li>
</ul>
<p>Elasticsearch 基于 Java 编写实现，Kibana 使用的是 node.js 框架，Fluentd 使用Ruby语言编写。</p>
<h2 id="部署efk" tabindex="-1"><a class="header-anchor" href="#部署efk"><span>部署EFK</span></a></h2>
<p>我们使用Docker来部署EFK，首先，让我们先编写一个Docker-Compose的配置文件：</p>
...
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>然后，我们使用以下命令来创建Docker容器，并且在后台运行：</p>
...
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><p>这还不算完，我们还需要修改Fluentd的配置，原始的配置，日志只记录在Fluentd本地的文本文件里面。我们需要修改配置，以使之能够将日志输入到ElasticSearch。</p>
<p>原始的配置文件如下所示：</p>
...
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>我们需要将<code v-pre>&lt;match **&gt;</code>节点修改为如下配置：</p>
...
<span class="line"><span class="token tag"><span class="token tag"><span class="token punctuation">&lt;/</span>match</span><span class="token punctuation">></span></span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>上述配置，实际上是利用了<code v-pre>fluent-plugin-elasticsearch</code>插件，将日志导入到ElasticSearch。</p>
<h2 id="kratos微服务对接efk" tabindex="-1"><a class="header-anchor" href="#kratos微服务对接efk"><span>Kratos微服务对接EFK</span></a></h2>
<p>首先，我们需要下载Fluent的一个封装：</p>
<div class="language-bash line-numbers-mode" data-highlighter="prismjs" data-ext="sh"><pre v-pre><code><span class="line">go get github.com/go-kratos/kratos/contrib/log/fluent/v2</span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><p>然后创建日志记录器：</p>
<div class="language-go line-numbers-mode" data-highlighter="prismjs" data-ext="go"><pre v-pre><code><span class="line">fluentLogger <span class="token string">"github.com/go-kratos/kratos/contrib/log/fluent/v2"</span></span>
<span class="line"></span>
<span class="line"><span class="token comment">// NewFluentLogger 创建一个日志记录器 - Fluent</span></span>
<span class="line"><span class="token keyword">func</span> <span class="token function">NewFluentLogger</span><span class="token punctuation">(</span>endpoint <span class="token builtin">string</span><span class="token punctuation">)</span> log<span class="token punctuation">.</span>Logger <span class="token punctuation">{</span></span>
<span class="line">    wrapped<span class="token punctuation">,</span> err <span class="token operator">:=</span> fluentLogger<span class="token punctuation">.</span><span class="token function">NewLogger</span><span class="token punctuation">(</span>endpoint<span class="token punctuation">)</span></span>
<span class="line">    <span class="token keyword">if</span> err <span class="token operator">!=</span> <span class="token boolean">nil</span> <span class="token punctuation">{</span></span>
<span class="line">        <span class="token function">panic</span><span class="token punctuation">(</span><span class="token string">"create fluent logger failed"</span><span class="token punctuation">)</span></span>
<span class="line">        <span class="token keyword">return</span> <span class="token boolean">nil</span></span>
<span class="line">    <span class="token punctuation">}</span></span>
<span class="line">    <span class="token keyword">return</span> wrapped</span>
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>现在，所有Kratos的日志就都注入到了EFK当中去了。</p>
<h2 id="kibana查询日志" tabindex="-1"><a class="header-anchor" href="#kibana查询日志"><span>Kibana查询日志</span></a></h2>
<p>Kibana的访问端口是5601，因此我们可以访问：<a href="http://127.0.0.1:5601/" target="_blank" rel="noopener noreferrer">http://127.0.0.1:5601/</a></p>
<p>当我们第一次进入Kibana，一片空白，什么都没有。那么，我们需要添加<code v-pre>Data View</code>，相当于创建了一个日志的查询视图。</p>
<p>我们点击：<code v-pre>Discover</code> -&gt; <code v-pre>Create a data view</code>，之后，我们将看到如下的界面：</p>
<p><img src="/assets/images/efk/kibana_create_data_view.png" alt="kibana_create_data_view"></p>
<p>在上面，我们在fluent里面设置了elasticsearch的索引为：fluent，所以，我们在<code v-pre>Index pattern</code>里面填写<code v-pre>fluent</code>，即可。<code v-pre>Timestamp field</code>我们可以从下拉框里面找到<code v-pre>ts</code>，选中即可。然后，我们就可以点击<code v-pre>Save data view to Kibana</code>创建视图了。</p>
<p>我们将看到日志查询的视图：</p>
<p><img src="/assets/images/efk/kibana_query_log.png" alt="kibana_query_log"></p>
<p>在视图里面，我们看到的还只是最原始的日志信息。我们可以在左边勾选<code v-pre>msg</code>或者其他需要关注的字段，Kibana就会过滤出我们关注的信息：</p>
<p><img src="/assets/images/efk/kibana_select_fields.png" alt="kibana_query_log"></p>
<p>如果需要构建更加复杂的查询，那么可以在最上面的搜索栏里边构建查询语句。</p>
