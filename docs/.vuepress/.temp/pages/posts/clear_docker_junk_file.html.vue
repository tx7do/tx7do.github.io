<template><div><h1 id="清除docker产生的垃圾文件" tabindex="-1"><a class="header-anchor" href="#清除docker产生的垃圾文件"><span>清除Docker产生的垃圾文件</span></a></h1>
<p>通过命令<code v-pre>df -h</code>查看磁盘空间占用，发现Docker的overlay对磁盘的占用极高。</p>
<p>通过<code v-pre>docker system prune -a</code>命令清除掉Docker的无用镜像、缓存、挂载数据，也并没有太多的改善。</p>
<p>通过<code v-pre>du -h --max-depth=1</code>或者<code v-pre>du -sh *</code>命令查看大文件的占用情况。</p>
