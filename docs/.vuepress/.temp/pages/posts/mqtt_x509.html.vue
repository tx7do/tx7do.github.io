<template><div><h1 id="mqtt用x509进行认证" tabindex="-1"><a class="header-anchor" href="#mqtt用x509进行认证"><span>MQTT用X509进行认证</span></a></h1>
...
<p>Predecessor of PKCS#12，这种格式是二进制格式，且证书和私钥存在一个 PFX 文件中。一般用于 Windows 上的 IIS 服务器。改格式的文件一般会有一个密码用于保证私钥的安全。</p>
...
<h3 id="tls证书单向认证" tabindex="-1"><a class="header-anchor" href="#tls证书单向认证"><span>TLS证书单向认证</span></a></h3>
<p>只需要提供CA证书即可。</p>
<p>CA证书由emqx提供的，CA 证书文件下载地址：<a href="https://static.emqx.net/data/broker.emqx.io-ca.crt" target="_blank" rel="noopener noreferrer">https://static.emqx.net/data/broker.emqx.io-ca.crt</a></p>
<div class="language-go line-numbers-mode" data-highlighter="prismjs" data-ext="go"><pre v-pre><code><span class="line"><span class="token keyword">func</span> <span class="token function">NewTlsConfig</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token operator">*</span>tls<span class="token punctuation">.</span>Config <span class="token punctuation">{</span></span>
...
<span class="line"><span class="token punctuation">}</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>mosquitto订阅</p>
<div class="language-bash line-numbers-mode" data-highlighter="prismjs" data-ext="sh"><pre v-pre><code><span class="line">mosquitto_sub <span class="token parameter variable">-h</span> <span class="token number">192.168</span>.111.100 <span class="token parameter variable">-p</span> <span class="token number">8883</span> <span class="token parameter variable">-t</span> <span class="token string">"/topic/UpdateTemperature"</span> <span class="token parameter variable">--cafile</span> /ca.crt</span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><p>mosquitto发布</p>
<div class="language-bash line-numbers-mode" data-highlighter="prismjs" data-ext="sh"><pre v-pre><code><span class="line">mosquitto_pub <span class="token parameter variable">-h</span> <span class="token number">192.168</span>.111.100 <span class="token parameter variable">-p</span> <span class="token number">8883</span> <span class="token parameter variable">-t</span> <span class="token string">"/topic/UpdateTemperature"</span> <span class="token parameter variable">-m</span> <span class="token string">"temperature:15"</span>  <span class="token parameter variable">--cafile</span> /ca.crt</span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><h3 id="mtls证书双向认证" tabindex="-1"><a class="header-anchor" href="#mtls证书双向认证"><span>mTLS证书双向认证</span></a></h3>
<p>不仅需要提供CA证书，还需要提供客户端的证书和私钥。</p>
<p>照理说，双向认证之后，MQTT服务器就不需要再进行用户名和密码的校验了，但是现实是，更多的时候，部署的时候，我们并不是直接在MQTT服务器上做SSL的支持，而是在此之前加一个代理服务器，比如HAProxy之类的，这样就只会在代理服务器上做SSL的支持，而不会在MQTT服务器上做SSL的支持。这样的话，我们在MQTT服务器上可能获取不到客户端的TLS证书，就更不要提获取到证书中的用户信息了。所以，至少还是需要做一下用户名的校验。</p>
<div class="language-go line-numbers-mode" data-highlighter="prismjs" data-ext="go"><pre v-pre><code><span class="line"><span class="token keyword">func</span> <span class="token function">NewMTLSConfig</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token operator">*</span>tls<span class="token punctuation">.</span>Config <span class="token punctuation">{</span></span>
<span class="line">	certpool <span class="token operator">:=</span> x509<span class="token punctuation">.</span><span class="token function">NewCertPool</span><span class="token punctuation">(</span><span class="token punctuation">)</span></span>
...
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><h3 id="完整代码" tabindex="-1"><a class="header-anchor" href="#完整代码"><span>完整代码</span></a></h3>
...
<li><a href="https://blog.csdn.net/mayue_web/article/details/121249082" target="_blank" rel="noopener noreferrer">mosquitto 测试MQTT TLS单向认证和双向认证</a></li>
<li><a href="https://openest.io/en/2020/01/03/mqtts-how-to-use-mqtt-with-tls/" target="_blank" rel="noopener noreferrer">MQTTS : How to use MQTT with TLS?</a></li>
</ul>
</div></template>
