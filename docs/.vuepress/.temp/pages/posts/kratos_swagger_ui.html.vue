<template><div><h1 id="golang微服务框架kratos轻松集成并使用swagger-ui" tabindex="-1"><a class="header-anchor" href="#golang微服务框架kratos轻松集成并使用swagger-ui"><span>Golang微服务框架Kratos轻松集成并使用Swagger UI</span></a></h1>
...
<h2 id="什么是-openapi" tabindex="-1"><a class="header-anchor" href="#什么是-openapi"><span>什么是 OpenAPI</span></a></h2>
<p>OpenAPI 是编写 RESTful API 的全球标准。它是一种规范，使得全球开发人员可以标准化 API 的设计，并在从头开始编写 REST API 时遵守所有安全、版本控制、错误处理和其他最佳实践。不仅仅是从头开始，即使现有的 API 也可以进行微调以符合全球标准。</p>
<p>此外，遵守开发产品的通用标准显然有助于什么。</p>
<p>最初，OpenAPI 被称为 Swagger 规范。Swagger 提出了构建 API 的最佳实践，然后这些最佳实践成为了 OpenAPI规范。</p>
<p>像 SwaggerHub 这样的工具可以帮助开发人员在基于浏览器的编辑器中构建 API，符合标准并完全控制设计过程。</p>
<p>使用 Swagger Inspector 等工具，我们还可以生成自己的 API 规范，并将其传递给组织中的其他团队。</p>
<p>需进一步了解，可查看 <a href="https://openapi.apifox.cn/" target="_blank" rel="noopener noreferrer">OpenAPI 规范（中文版）</a>。</p>
<h2 id="什么是-swagger" tabindex="-1"><a class="header-anchor" href="#什么是-swagger"><span>什么是 Swagger</span></a></h2>
<p>OpenAPI 是一个编写 API 文档的规范，然而如果手动去编写 OpenAPI 规范的文档，是非常麻烦的。而 Swagger 就是一个实现了OpenAPI 规范的工具集。</p>
<p>官网：<a href="https://swagger.io/" target="_blank" rel="noopener noreferrer">https://swagger.io/</a></p>
<p>Swagger 包含的工具集：</p>
<ul>
<li><strong>Swagger编辑器</strong>： Swagger Editor允许在浏览器中编辑YAML中的OpenAPI规范并实时预览文档。</li>
...
<h2 id="怎么样集成swagger到kratos" tabindex="-1"><a class="header-anchor" href="#怎么样集成swagger到kratos"><span>怎么样集成Swagger到Kratos</span></a></h2>
<p>首先，我们要了解的是：要在项目中集成Swagger，唯一的办法就是通过Swagger UI来集成。</p>
<p>Swagger UI依赖读取的是OpenAPI的json或者yaml格式的API文档，这个文档不是给人来读取的，而是给Swagger UI。同样，它也不是给人来写的，靠的是生成器来生成的。</p>
<p>因此，下面首先我们需要来了解API是如何生成的，怎样生成的。</p>
<h3 id="api文档怎么产生呢" tabindex="-1"><a class="header-anchor" href="#api文档怎么产生呢"><span>API文档怎么产生呢？</span></a></h3>
<p>因为Kratos是依托于Protobuf和gRPC来设计API的，所以，我们可以由相关的生成器工具来生成。</p>
<p>Protobuf是一个DSL语言，它需要一个叫做protoc的工具来将API编译转换成目标语言。而它的具体工具是依靠插件来实现的。</p>
<p>目前已有的由Go编写的OpenAPI生成插件有两个：</p>
<ul>
<li>OpenAPIv2生成器：<a href="http://github.com/grpc-ecosystem/grpc-gateway/v2/protoc-gen-openapiv2" target="_blank" rel="noopener noreferrer">protoc-gen-openapiv2</a></li>
<li>OpenAPIv3生成器：<a href="http://github.com/google/gnostic/cmd/protoc-gen-openapi" target="_blank" rel="noopener noreferrer">protoc-gen-openapi</a></li>
</ul>
<p>我们可以通过以下命令来安装：</p>
<div class="language-bash line-numbers-mode" data-highlighter="prismjs" data-ext="sh"><pre v-pre><code><span class="line">go <span class="token function">install</span> github.com/grpc-ecosystem/grpc-gateway/v2/protoc-gen-openapiv2@latest</span>
...
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div></div></div><p>直接生成的命令倒是简单。</p>
<p>生成 OpenAPI v2 json文档：</p>
...
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><p>生成 OpenAPI v3 yaml文档：</p>
...
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><p>但是，直接使用命令是很不方便的，我们可以使用一个叫做<a href="https://buf.build/" target="_blank" rel="noopener noreferrer">Buf.Build</a>来进行工程化生成。</p>
<p>安装Buf很简单：</p>
...
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><p>然后我们需要在项目的根目录下创建<code v-pre>buf.work.yaml</code>、<code v-pre>buf.yaml</code>、<code v-pre>buf.gen.yaml</code>等配置文件。关于这些配置文件如何进行配置，本文将不做详细的讲解，可自行寻找教程学习。本文只着重讲解生成OpenAPI的相关知识。</p>
<p>要生成OpenAPI，我们还需要创建一个配置文件<code v-pre>buf.openapi.gen.yaml</code>，并且放在proto同级目录下，<strong>在这里我们以后台API做讲解</strong>：</p>
...
<span class="line"><span class="token comment">#    out: ../docs</span></span>
...
<span class="line">      <span class="token punctuation">-</span> naming=json</span>
<span class="line">      <span class="token punctuation">-</span> depth=2</span>
<span class="line">      <span class="token punctuation">-</span> paths=source_relative</span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>接着我们就可以在项目根目录下使用下面的命令来生成了：</p>
...
<h3 id="怎么将openapi-yaml文件引入到swagger-ui" tabindex="-1"><a class="header-anchor" href="#怎么将openapi-yaml文件引入到swagger-ui"><span>怎么将openapi.yaml文件引入到Swagger UI</span></a></h3>
...
<p>最后，我们就需要来集成Swagger UI进来了。我为此封装了一个项目，要使用它，我们需要：</p>
...
<p>如果API服务的端口是8080，那么我们可以访问链接来访问Swagger UI：</p>
...
<p>作为程序员，偷懒是一个美德，因为要想偷懒，就必须要想尽办法改善流程，制造工具，能自动化的就自动化。而集成Swagger UI在工作中也确实切实的减轻了我和团队的工作量，提升了开发效率。</p>
<p>如果你对Kratos集成Swagger UI还有什么不太明白的，我有一个开源的CMS项目，里面有完整的应用代码：</p>
<ul>
<li><a href="https://github.com/tx7do/kraots-cms" target="_blank" rel="noopener noreferrer">https://github.com/tx7do/kraots-cms</a></li>
<li><a href="https://gitee.com/tx7do/kraots-cms" target="_blank" rel="noopener noreferrer">https://gitee.com/tx7do/kraots-cms</a></li>
</ul>
