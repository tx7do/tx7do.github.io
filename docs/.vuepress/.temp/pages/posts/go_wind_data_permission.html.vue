<template><div><h1 id="gowind-admin-风行-—-开箱即用的企业级全栈中后台框架-数据权限体系设计与实现" tabindex="-1"><a class="header-anchor" href="#gowind-admin-风行-—-开箱即用的企业级全栈中后台框架-数据权限体系设计与实现"><span>GoWind Admin｜风行 — 开箱即用的企业级全栈中后台框架：数据权限体系设计与实现</span></a></h1>
...
<h2 id="一、数据权限的五大核心粒度层级" tabindex="-1"><a class="header-anchor" href="#一、数据权限的五大核心粒度层级"><span>一、数据权限的五大核心粒度层级</span></a></h2>
...
<h3 id="_1-租户级-tenant-level・顶层全局隔离" tabindex="-1"><a class="header-anchor" href="#_1-租户级-tenant-level・顶层全局隔离"><span>1. 租户级（Tenant-level・顶层全局隔离）</span></a></h3>
<p><strong>所有数据权限的前置基础</strong>，基于<code v-pre>tenant_id</code>实现租户间的全局数据隔离，是多租户 SaaS 架构的核心能力，也是企业级框架的必备基础。</p>
<ul>
...
<ul>
<li>框架要求<strong>所有业务表默认内置<code v-pre>tenant_id</code>字段</strong>，作为租户数据的唯一标识；</li>
<li>通过 ORM 层<strong>全局非侵入式拦截</strong>，所有 SQL 查询 / 写入操作<strong>自动强制注入<code v-pre>tenant_id = '当前租户ID'</code></strong>，开发者无需手动处理；</li>
<li>支持<strong>逻辑隔离（单库单表）</strong> 和 <strong>物理隔离（分库分表）</strong> 两种模式，可通过配置灵活切换，适配不同租户数据量与安全等级需求。</li>
</ul>
</li>
</ul>
<blockquote>
<p>✅ 核心优势：租户间数据绝对隔离，满足 SaaS 平台的核心合规要求；<code v-pre>tenant_id</code>全链路自动注入，无侵入式开发；单 / 多租户模式一键切换，适配企业从私有化部署到 SaaS 化运营的全生命周期。</p>
<p>⚠️ 关键设计：超级管理员可配置是否开启 “跨租户访问”，默认关闭，从源头杜绝租户间数据泄露。</p>
</blockquote>
<h3 id="_2-业务单元级-粗粒度・主流首选" tabindex="-1"><a class="header-anchor" href="#_2-业务单元级-粗粒度・主流首选"><span>2. 业务单元级（粗粒度・主流首选）</span></a></h3>
...
<li><strong>实现方式</strong>：框架通过非侵入式拦截，在 SQL 查询阶段自动注入业务单元过滤条件，如 <code v-pre>WHERE dept_id = '当前用户部门ID'</code>、<code v-pre>project_id = '当前用户所属项目ID'</code>，无需开发者手动处理。</li>
</ul>
<blockquote>
<p>✅ 核心优势：权限逻辑清晰易懂，用户易理解、管理员易配置；查询过滤性能高效，无额外性能损耗，适配 80% 以上的企业级中后台常规场景。</p>
</blockquote>
<h3 id="_3-行级-row-level・记录级管控" tabindex="-1"><a class="header-anchor" href="#_3-行级-row-level・记录级管控"><span>3. 行级（Row-level・记录级管控）</span></a></h3>
...
<li><strong>实现方式</strong>：框架内置数据归属字段自动填充能力，每条记录默认携带<code v-pre>created_by</code>（创建人）、<code v-pre>assigned_to</code>（负责人）等归属字段，查询时强制校验当前用户身份并自动追加过滤条件。</li>
</ul>
<blockquote>
<p>⚠️ 关键注意：框架通过统一的 ORM 拦截机制确保数据写入时自动绑定归属人，从源头避免手动开发导致的归属字段缺失问题，彻底杜绝权限漏洞。</p>
</blockquote>
<h3 id="_4-列级-column-level・字段级脱敏-隐藏" tabindex="-1"><a class="header-anchor" href="#_4-列级-column-level・字段级脱敏-隐藏"><span>4. 列级（Column-level・字段级脱敏 / 隐藏）</span></a></h3>
<p>对本租户内数据记录中的敏感字段实施差异化展示与访问控制，是满足数据隐私保护合规要求的核心能力。</p>
<ul>
<li><strong>粒度描述</strong>：基于用户角色 / 权限等级差异化展示字段内容，高权限角色可查看完整敏感信息（如薪资、手机号、银行卡号），普通角色仅查看脱敏内容（如<code v-pre>138****0000</code>）或直接隐藏字段。</li>
...
<h3 id="_5-操作级-状态级-最细粒度・动态上下文管控" tabindex="-1"><a class="header-anchor" href="#_5-操作级-状态级-最细粒度・动态上下文管控"><span>5. 操作级 / 状态级（最细粒度・动态上下文管控）</span></a></h3>
...
<h2 id="二、datascope-行级数据权限的统一实现引擎" tabindex="-1"><a class="header-anchor" href="#二、datascope-行级数据权限的统一实现引擎"><span>二、DataScope：行级数据权限的统一实现引擎</span></a></h2>
...
<h3 id="_1-租户级基础过滤-全局前置・默认开启" tabindex="-1"><a class="header-anchor" href="#_1-租户级基础过滤-全局前置・默认开启"><span>1. 租户级基础过滤（全局前置・默认开启）</span></a></h3>
<p><strong>DataScope 的前置核心过滤维度</strong>，所有行级权限控制均在<strong>租户隔离</strong>的基础上生效，是框架默认开启的强制过滤规则。</p>
<ul>
<li><strong>核心能力</strong>：无需任何配置，框架自动为所有 SQL 查询注入<code v-pre>tenant_id = '当前租户ID'</code>，确保用户仅能访问本租户内的数据；</li>
...
<blockquote>
<p>💡设计亮点：<code v-pre>tenant_id</code>的过滤优先级高于所有其他 DataScope 维度，即使配置了 “全部数据” 权限，用户也仅能查看<strong>本租户内</strong>的全量数据，从引擎层杜绝租户间数据泄露。</p>
</blockquote>
<h3 id="_2-组织架构级-部门-机构・核心主流" tabindex="-1"><a class="header-anchor" href="#_2-组织架构级-部门-机构・核心主流"><span>2. 组织架构级（部门 / 机构・核心主流）</span></a></h3>
...
<tr>
<td>全部数据</td>
<td>无任何数据过滤，可访问<strong>本租户内</strong>系统全量数据</td>
<td>租户内超级管理员、租户系统管理员</td>
</tr>
<tr>
<td>本部门数据</td>
<td>仅可查看<strong>本租户内</strong>当前用户所属直接部门的所有数据</td>
<td>部门普通员工、部门专员</td>
</tr>
<tr>
<td>本部门及子部门数据</td>
<td>可查看<strong>本租户内</strong>当前部门及其所有下级部门的全量数据（自动递归组织树）</td>
<td>部门经理、部门负责人</td>
</tr>
<tr>
<td>自定义部门数据</td>
<td>可手动配置<strong>本租户内</strong>任意一个或多个部门组合，仅访问指定部门数据</td>
<td>租户内公司高管、跨部门协调岗</td>
</tr>
</tbody>
</table>
<blockquote>
...
<h3 id="_3-个人级-owner-level・归属者管控" tabindex="-1"><a class="header-anchor" href="#_3-个人级-owner-level・归属者管控"><span>3. 个人级（Owner-level・归属者管控）</span></a></h3>
<p>实现 “我的数据我做主” 的最小权限单元，精准适配本租户内强归属型数据的权限管控需求。</p>
<ul>
<li><strong>核心能力</strong>：根据数据归属关系自动追加过滤条件，如<code v-pre>user_id = ?</code>、<code v-pre>created_by = ?</code>，仅允许数据创建人、归属人访问。</li>
<li><strong>典型适用场景</strong>：个人工单、报销申请单、个人笔记、员工打卡记录、个人客户跟进记录等强个人归属型数据。</li>
<li><strong>开发优势</strong>：框架内置归属字段自动填充功能，数据写入时无需手动设置<code v-pre>created_by</code>等字段，由 ORM 框架统一拦截处理，从源头保证数据归属的准确性。</li>
</ul>
<h3 id="_4-角色级-role-based-scope・批量管控" tabindex="-1"><a class="header-anchor" href="#_4-角色级-role-based-scope・批量管控"><span>4. 角色级（Role-based Scope・批量管控）</span></a></h3>
...
<h3 id="_5-自定义-sql-拼接级-高扩展・业务适配" tabindex="-1"><a class="header-anchor" href="#_5-自定义-sql-拼接级-高扩展・业务适配"><span>5. 自定义 SQL 拼接级（高扩展・业务适配）</span></a></h3>
<p>DataScope 的底层基于动态 SQL 注入技术构建，具备极强的业务适配性，支持基于<strong>本租户内</strong>任意业务字段的个性化权限过滤。</p>
<ul>
<li><strong>核心能力</strong>：可基于企业任意业务字段进行数据范围过滤，如<code v-pre>project_id</code>（项目 ID）、<code v-pre>warehouse_id</code>（仓库 ID）、<code v-pre>region_id</code>（区域 ID）等；</li>
...
<h3 id="_6-动态属性级-abac-扩展・高安全场景" tabindex="-1"><a class="header-anchor" href="#_6-动态属性级-abac-扩展・高安全场景"><span>6. 动态属性级（ABAC 扩展・高安全场景）</span></a></h3>
...
<li><strong>扩展方式</strong>：框架提供标准化的<code v-pre>PolicyEvaluator</code>（策略评估器）接口，开发者只需实现接口的核心评估方法，即可注入自定义的动态权限规则，实现 “运行时实时权限决策”；</li>
<li><strong>未来演进</strong>：框架将原生支持与 Open Policy Agent (OPA) 等主流外部策略引擎的集成，构建统一的零信任安全架构，适配超高级别的安全管控需求。</li>
</ul>
<h2 id="三、框架级优势-让数据权限从-开发负担-变为-核心能力" tabindex="-1"><a class="header-anchor" href="#三、框架级优势-让数据权限从-开发负担-变为-核心能力"><span>三、框架级优势：让数据权限从 “开发负担” 变为 “核心能力”</span></a></h2>
<p>GoWind Admin 的数权限体系并非简单的功能堆砌，而是从企业级开发、运维、合规三大维度进行框架级设计，真正实现数据权限的 “开箱即用、低码开发、灵活扩展”：</p>
<ol>
<li><strong>租户隔离原生支持</strong>：内置<code v-pre>tenant_id</code>全链路自动注入机制，支持逻辑 / 物理两种隔离模式，单 / 多租户一键切换，完美适配私有化部署与 SaaS 平台建设；</li>
<li><strong>非侵入式开发</strong>：所有权限过滤均通过 AOP 拦截、动态 SQL 注入实现，业务代码与权限代码完全解耦，开发者无需关注权限细节，专注业务逻辑开发；</li>
<li><strong>全链路自动化</strong>：从数据写入时的<code v-pre>tenant_id</code>、归属字段自动填充，到查询时的租户 + 权限条件自动注入，再到权限变更时的实时同步，框架实现全链路自动化处理，减少手动操作，杜绝权限漏洞；</li>
