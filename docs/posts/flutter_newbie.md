...
import 'package:intl/date_symbol_data_local.dart';
import 'package:intl/format.dart';
...
但是，这个也只是看起来很好，就目前而言，会产生一个性能上的问题：官方推荐每一个Widget最好是const的，而用function创建的Widget是不能够加const的，这样的话就会產生一個頻繁內存分配的性能問題。

基於StatelessWidget封裝一個新的Widget將會是更好的選擇。
