...
class XMLToSitemapItemStream extends stream_1.Transform {
        ...
        this.saxStream.on('text', (text) => {
            switch (currentTag) {
                ...
                case types_1.TagNames.loc:
                    // Validate URL
                    if (text.length > constants_1.LIMITS.MAX_URL_LENGTH) {
                        this.logger('warn', `URL exceeds max length of ${constants_1.LIMITS.MAX_URL_LENGTH}: ${text.substring(0, 100)}...`);
                        this.err(`URL exceeds max length of ${constants_1.LIMITS.MAX_URL_LENGTH}`);
                    }
                    else if (!constants_1.LIMITS.URL_PROTOCOL_REGEX.test(text)) {
                ...
                case types_1.TagNames.changefreq:
                    if ((0, validation_1.isValidChangeFreq)(text)) {
                ...
                case types_1.TagNames.lastmod:
                    if (constants_1.LIMITS.ISO_DATE_REGEX.test(text)) {
                ...
                case types_1.TagNames['video:tag']:
                    if (currentVideo.tag.length < constants_1.LIMITS.MAX_TAGS_PER_VIDEO) {
                        currentVideo.tag.push(text);
                    }
                    else {
                        this.logger('warn', `video has too many tags (max ${constants_1.LIMITS.MAX_TAGS_PER_VIDEO})`);
                        this.err(`video has too many tags (max ${constants_1.LIMITS.MAX_TAGS_PER_VIDEO})`);
                    }
                    break;
                ...
                case types_1.TagNames['video:requires_subscription']:
                    if ((0, validation_1.isValidYesNo)(text)) {
                ...
                case types_1.TagNames['video:title']:
                    if (currentVideo.title.length + text.length <=
                        constants_1.LIMITS.MAX_VIDEO_TITLE_LENGTH) {
                        currentVideo.title += text;
                    }
                    else {
                        this.logger('warn', `video title exceeds max length of ${constants_1.LIMITS.MAX_VIDEO_TITLE_LENGTH}`);
                        this.err(`video title exceeds max length of ${constants_1.LIMITS.MAX_VIDEO_TITLE_LENGTH}`);
                    }
                    break;
                case types_1.TagNames['video:description']:
                    if (currentVideo.description.length + text.length <=
                        constants_1.LIMITS.MAX_VIDEO_DESCRIPTION_LENGTH) {
                        currentVideo.description += text;
                    }
                    else {
                        this.logger('warn', `video description exceeds max length of ${constants_1.LIMITS.MAX_VIDEO_DESCRIPTION_LENGTH}`);
                        this.err(`video description exceeds max length of ${constants_1.LIMITS.MAX_VIDEO_DESCRIPTION_LENGTH}`);
                    }
                    break;
                case types_1.TagNames['news:name']:
                    ...
                    if (currentItem.news.publication.name.length + text.length <=
                        constants_1.LIMITS.MAX_NEWS_NAME_LENGTH) {
                        currentItem.news.publication.name += text;
                    }
                    else {
                        this.logger('warn', `news name exceeds max length of ${constants_1.LIMITS.MAX_NEWS_NAME_LENGTH}`);
                        this.err(`news name exceeds max length of ${constants_1.LIMITS.MAX_NEWS_NAME_LENGTH}`);
                    }
                    break;
                case types_1.TagNames['news:title']:
                    ...
                    if (currentItem.news.title.length + text.length <=
                        constants_1.LIMITS.MAX_NEWS_TITLE_LENGTH) {
                        currentItem.news.title += text;
                    }
                    else {
                        this.logger('warn', `news title exceeds max length of ${constants_1.LIMITS.MAX_NEWS_TITLE_LENGTH}`);
                        this.err(`news title exceeds max length of ${constants_1.LIMITS.MAX_NEWS_TITLE_LENGTH}`);
                    }
                    break;
                case types_1.TagNames['image:caption']:
                    if (!currentImage.caption) {
                        currentImage.caption =
                            text.length <= constants_1.LIMITS.MAX_IMAGE_CAPTION_LENGTH
                                ? text
                                : text.substring(0, constants_1.LIMITS.MAX_IMAGE_CAPTION_LENGTH);
                        if (text.length > constants_1.LIMITS.MAX_IMAGE_CAPTION_LENGTH) {
                            this.logger('warn', `image caption exceeds max length of ${constants_1.LIMITS.MAX_IMAGE_CAPTION_LENGTH}`);
                            this.err(`image caption exceeds max length of ${constants_1.LIMITS.MAX_IMAGE_CAPTION_LENGTH}`);
                        }
                    }
                    else if (currentImage.caption.length + text.length <=
                        constants_1.LIMITS.MAX_IMAGE_CAPTION_LENGTH) {
                ...
                case types_1.TagNames['image:title']:
                    if (!currentImage.title) {
                        currentImage.title =
                            text.length <= constants_1.LIMITS.MAX_IMAGE_TITLE_LENGTH
                                ? text
                                : text.substring(0, constants_1.LIMITS.MAX_IMAGE_TITLE_LENGTH);
                        if (text.length > constants_1.LIMITS.MAX_IMAGE_TITLE_LENGTH) {
                            this.logger('warn', `image title exceeds max length of ${constants_1.LIMITS.MAX_IMAGE_TITLE_LENGTH}`);
                            this.err(`image title exceeds max length of ${constants_1.LIMITS.MAX_IMAGE_TITLE_LENGTH}`);
                        }
                    }
                    else if (currentImage.title.length + text.length <=
                        constants_1.LIMITS.MAX_IMAGE_TITLE_LENGTH) {
        ...
        this.saxStream.on('attribute', (attr) => {
            switch (currentTag) {
                ...
                case types_1.TagNames['video:id']:
                    break;
                case types_1.TagNames['video:restriction']:
                    if (attr.name === 'relationship' && (0, validation_1.isAllowDeny)(attr.value)) {
                    ...
                    break;
                case types_1.TagNames['video:price']:
                    if (attr.name === 'type' && (0, validation_1.isPriceType)(attr.value)) {
                        currentVideo['price:type'] = attr.value;
                    }
                    else if (attr.name === 'currency') {
                        currentVideo['price:currency'] = attr.value;
                    }
                    else if (attr.name === 'resolution' && (0, validation_1.isResolution)(attr.value)) {
                ...
                case types_1.TagNames['video:player_loc']:
                    ...
                    else if (attr.name === 'allow_embed' && (0, validation_1.isValidYesNo)(attr.value)) {
        ...
        this.saxStream.on('closetag', (tag) => {
            switch (tag) {
                case types_1.TagNames.url:
                    this.urlCount++;
                    if (this.urlCount > constants_1.LIMITS.MAX_URL_ENTRIES) {
                        this.logger('error', `Sitemap exceeds maximum of ${constants_1.LIMITS.MAX_URL_ENTRIES} URLs`);
                        this.err(`Sitemap exceeds maximum of ${constants_1.LIMITS.MAX_URL_ENTRIES} URLs`);
                ...
                case types_1.TagNames['video:video']:
                    if (currentItem.video.length < constants_1.LIMITS.MAX_VIDEOS_PER_URL) {
                        currentItem.video.push(currentVideo);
                    }
                    else {
                        this.logger('warn', `URL has too many videos (max ${constants_1.LIMITS.MAX_VIDEOS_PER_URL})`);
                        this.err(`URL has too many videos (max ${constants_1.LIMITS.MAX_VIDEOS_PER_URL})`);
                    }
                    currentVideo = videoTemplate();
                    break;
                case types_1.TagNames['image:image']:
                    if (currentItem.img.length < constants_1.LIMITS.MAX_IMAGES_PER_URL) {
                        currentItem.img.push(currentImage);
                    }
                    else {
                        this.logger('warn', `URL has too many images (max ${constants_1.LIMITS.MAX_IMAGES_PER_URL})`);
                        this.err(`URL has too many images (max ${constants_1.LIMITS.MAX_IMAGES_PER_URL})`);
                    }
                    currentImage = { ...imageTemplate };
                    break;
                case types_1.TagNames['xhtml:link']:
                    if (!dontpushCurrentLink) {
                        if (currentItem.links.length < constants_1.LIMITS.MAX_LINKS_PER_URL) {
                            currentItem.links.push(currentLink);
                        }
                        else {
                            this.logger('warn', `URL has too many links (max ${constants_1.LIMITS.MAX_LINKS_PER_URL})`);
                            this.err(`URL has too many links (max ${constants_1.LIMITS.MAX_LINKS_PER_URL})`);
                        }
                    }
    ...
    _transform(data, encoding, callback) {
        try {
            const cb = () => callback(this.level === types_1.ErrorLevel.THROW && this.errors.length > 0
                ? this.errors[0]
                : null);
    ...
    err(msg) {
        this.errorCount++;
        if (this.errors.length < constants_1.LIMITS.MAX_PARSER_ERRORS) {
